...
if (
  ...
  !["install", "uninstall", "status", ...(process.platform === "win32" ? ["run"] : [])].includes(
    command,
  )
) {
  console.error(
...
if (process.platform === "win32") {
  ...
  if (process.env.SYNARA_MOBILE_HOME)
    throw new Error(
      "Windows login service uses %USERPROFILE%\\.synara-mobile; use server.mjs for a custom mobile home",
    );
  const { windowsService } = await import("./lib/windows-service.mjs");
  await windowsService({
    command,
    origin: options["--origin"],
    directory: join(homedir(), ".synara-mobile"),
