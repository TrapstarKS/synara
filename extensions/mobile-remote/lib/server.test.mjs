...
test(
  ...
  async (t) => {
    ...
    const upstream = http.createServer((req, res) => {
      ...
      if (requestUrl.pathname === "/headers") {
    ...
    assert.equal(proxiedRequest.headers.cookie, "synara-session=target");
    assert.equal(proxiedRequest.headers.origin, `http://127.0.0.1:${upstream.address().port}`);
