...
The companion serves `/mobile` and injects a manifest, service worker registration, theme-token bridge, and a preferences link into the proxied HTML. It discovers the installed `Synara.app` backend, verifies that the process owns its loopback listener, and supplies the desktop's short-lived WebSocket credential only on the Mac-side hop. Port and credential changes are picked up after an app restart. Conversation rendering, sending messages, tool approvals, interruption, and provider behavior remain owned by that same Synara instance.
...
Prerequisites: Node 24.13.1+, `Synara.app` installed and open, Tailscale on both devices, tailnet HTTPS enabled.
...
The automatic desktop bridge is macOS-only. An explicit upstream override must accept the read-only companion connection from loopback; use it only for development or tests. The browser proxy preserves Synara's authentication policy and never sends the desktop credential to the phone.
...
Local Apple Silicon releases use the persistent ad-hoc `TrapRAM Signing` identity in the owner's login Keychain so updates keep the same macOS code-signing identity. Before publication, verify leaf SHA-1 `0a15f0539b0f02a8d880e637b0c2744f1f0b5dfb`, select the identity explicitly with `CSC_NAME=TrapRAM Signing`, and use `SYNARA_MAC_SIGNING_MODE=adhoc`. GitHub Actions only syncs upstream source; it does not test, build, or publish releases. This certificate is not notarized, so macOS may require `xattr -cr /Applications/Synara.app` once after the first DMG download, just as with TrapRAM.
