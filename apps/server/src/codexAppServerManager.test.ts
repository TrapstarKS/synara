...
describe("handleServerNotification error normalization", () => {
  ...
  it.each(["thread/closed", "thread/status/changed"])(
    "settles only the unloaded child's pending turn on %s",
    (method) => {
      const { manager, context, emitEvent, updateSession } = createCollabNotificationHarness();
      const cancelTurn = vi.fn(() => Promise.resolve());
      Object.assign(context, {
      ...
      notify("turn/started", "child", "old-child-turn");
      notify("turn/started", "child", "child-turn");
      notify("turn/completed", "child", "old-child-turn");
      notify("turn/started", "sibling", "sibling-turn");
      handleServerNotificationForTest(manager, context, {
      ...
      expect(
        emitEvent.mock.calls.filter(
          ([event]) => (event as ProviderEvent).method === "turn/aborted",
        ),
      ).toEqual([]);
      emitEvent.mockClear();
      updateSession.mockClear();
      ...
      for (let attempt = 0; attempt < 2; attempt++) {
        handleServerNotificationForTest(manager, context, {
          method,
          params: { threadId: "child", status: { type: "notLoaded" } },
        });
      }
      ...
      expect(emitEvent).toHaveBeenLastCalledWith(
        expect.objectContaining({ method: "turn/completed", providerThreadId: "sibling" }),
      );
    },
  );

  it.each(["session/closed", "session/exited"])(
    "settles pending native descendants immediately when their runtime emits %s",
    async (method) => {
      vi.useFakeTimers();
      try {
        const { manager, context, emitEvent } = createCollabNotificationHarness();
        ...
        notify("turn/started", "provider_parent", "parent-turn");
        notify("turn/started", "child", "child-turn");
        notify("turn/started", "grandchild", "grandchild-turn");
        notify("turn/started", "done-child", "done-turn");
        notify("turn/completed", "provider_parent", "parent-turn");
        expect(
          emitEvent.mock.calls.filter(
            ([event]) => (event as ProviderEvent).method === "turn/aborted",
          ),
        ).toEqual([]);
        notify("turn/completed", "done-child", "done-turn");
        emitEvent.mockClear();
        const lifecycle = manager as unknown as {
          emitLifecycleEvent: (context: unknown, method: string, message: string) => void;
        };
        lifecycle.emitLifecycleEvent(context, method, "Runtime closed");
        lifecycle.emitLifecycleEvent(context, method, "Runtime closed");
        expect(
          emitEvent.mock.calls
            .map(([event]) => event as ProviderEvent)
            .filter((event) => event.method === "turn/aborted")
            .map((event) => [event.providerThreadId, event.turnId]),
        ).toEqual([
          ["child", "child-turn"],
          ["grandchild", "grandchild-turn"],
        ]);
        expect(emitEvent.mock.calls[0]?.[0]).toMatchObject({ method: "turn/aborted" });
        expect(emitEvent.mock.calls[2]?.[0]).toMatchObject({ method });
        await vi.advanceTimersByTimeAsync(500);
        expect(sendRequest.mock.calls.filter((call) => call[1] === "turn/start")).toEqual([]);
      } finally {
        vi.useRealTimers();
