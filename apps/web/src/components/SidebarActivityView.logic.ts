...
export function resolveActivityStatusGroup(thread: SidebarThreadSummary): ActivityStatusGroup {
  if (requiresActivityAttention(thread)) return "attention";
  if (isThreadRunningForActivity(thread)) return "running";
  if (hasUnseenCompletion(thread)) return "unseenCompleted";
  return "seen";
}
