...
- macOS metadata note:
  ...
  - The desktop build script reuses the builder's `.zip` when its Electron framework symlinks are intact. It verifies both the original and extracted app signatures, patches the matching `latest-mac*.yml` hash/size, and removes `.zip.blockmap` to retain full-archive updates. A legacy ZIP with missing or flattened framework symlinks is rebuilt with `ditto` and passes the same checks. Corrupt ZIPs or invalid signatures fail the build; they are not hidden by a repack.
