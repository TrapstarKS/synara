...
const upstream = new URL(process.env.SYNARA_MOBILE_UPSTREAM ?? "http://127.0.0.1:58090");
...
server.on("upgrade", (req, socket, head) => {
  ...
  const target = new WebSocket(upstream.origin.replace("http:", "ws:") + req.url, {
  ...
  target.once("open", () => {
    ...
    wss.handleUpgrade(req, socket, head, (client) => {
      ...
      target.on("close", () => client.close(1012, "Synara disconnected"));
    });
  });
});
