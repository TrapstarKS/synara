...
- Cache reads and writes for the latest model request and session totals in the context meter, when Codex reports them
...
Cache reads and writes are part of input and context usage; the displayed counts describe past requests and do not predict the next cache hit.
The tooltip also estimates how recent the last confirmed cache activity was. This is a recency hint, not a guarantee that the next request will reuse the cache.
