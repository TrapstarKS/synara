 ...
 * of earlier Mind migrations declared `project_id REFERENCES projection_projects
 ...
 * that did. Fresh installs skip every step because the current migrations no
 * longer declare the foreign key.
 */
export default Effect.gen(function* () {
  ...
  if (
    ...
    yield* sql`DROP TABLE IF EXISTS mind_profiles_v110`;
    yield* sql`CREATE TABLE mind_profiles_v110 (
      project_id TEXT PRIMARY KEY,
      text TEXT NOT NULL CHECK (length(text) BETWEEN 1 AND 500),
      opted_in INTEGER NOT NULL DEFAULT 0 CHECK (opted_in IN (0, 1)),
      updated_at TEXT NOT NULL
    )`;
    yield* sql`INSERT INTO mind_profiles_v110 SELECT * FROM mind_profiles`;
    yield* sql`DROP TABLE mind_profiles`;
    yield* sql`ALTER TABLE mind_profiles_v110 RENAME TO mind_profiles`;
  }
  ...
  if (
    ...
    yield* sql`DROP TABLE IF EXISTS mind_operation_receipts_v110`;
    yield* sql`CREATE TABLE mind_operation_receipts_v110 (
    ...
    yield* sql`INSERT INTO mind_operation_receipts_v110 SELECT * FROM mind_operation_receipts`;
    yield* sql`DROP TABLE mind_operation_receipts`;
    yield* sql`ALTER TABLE mind_operation_receipts_v110 RENAME TO mind_operation_receipts`;
  }
});
