...
const approvedAttributions: readonly ApprovedAttribution[] = [
  ...
  },
  {
    ...
    line: `- Discover actual launchd job configurations and paths; do not assume saved paths are still current. The initially installed jobs are \`${mobileBackendServiceName}\` (58090) and \`${mobileCompanionServiceName}\` (58091), with homes \`~/.synara-preview-synara\` and \`~/.synara-mobile\`. Tailscale HTTPS 8443 points to companion 58091. Never enable Funnel or replace other Serve routes.`,
  },
  {
  ...
  {
    path: "extensions/mobile-remote/service.mjs",
    line: `{ label: "${mobileCompanionServiceName}", port: 58091, file: join(root, "server.mjs") },`,
  },
];
