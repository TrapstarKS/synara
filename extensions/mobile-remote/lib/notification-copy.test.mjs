...
test("notifications explain the pending action or question, with task title and an open action", () => {
  const thread = {
    ...
    activities: [
      { kind: "approval.requested", turnId: "old", payload: { detail: "Wrong action" } },
      {
        ...
        turnId: "turn",
        payload: { requestId: "approve", requestKind: "command", detail: "bun run test" },
      },
      {
        ...
        payload: {
          requestId: "question",
          questions: [{ question: "Qual conta devo usar para testar o login?" }],
        },
      },
    ],
    pendingInteractions: [
  ...
  assert.deepEqual(notificationCopy(event("approval"), thread), {
    title: "Aprovação · Corrigir login",
    body: "Executar comando: bun run test",
    actionTitle: "Revisar ação",
  });
  assert.equal(
    notificationCopy(event("input"), thread).body,
    "Qual conta devo usar para testar o login?",
  );
...
test("completion previews use only the final assistant message from the matching turn", () => {
  const copy = notificationCopy(event("completed"), {
    messages: [
      { role: "assistant", turnId: "old", text: "Wrong turn" },
      {
        role: "assistant",
        turnId: "turn",
        text: "**Login corrigido.** [Testes](https://example.test) passaram.",
        streaming: false,
      },
  ...
  assert.equal(
    notificationCopy(event("failed"), { session: { lastError: "Conexão com o provedor expirou." } })
      .body,
  ...
  assert.doesNotMatch(
    preview("token=private-value Authorization: Bearer private-value"),
    /private-value/,
  );
  assert.equal(preview("Executar my_task --pattern '*.ts'"), "Executar my_task --pattern '*.ts'");
  assert.equal(
    notificationCopy(event("completed"), {
      latestTurn: { assistantMessageId: "final" },
      messages: [
    ...
    hasLiveBackgroundTasks(
      {
        activities: [
          ...
          { kind: "task.started", turnId: "turn", payload: { taskId: "worker" } },
        ],
      },
      "turn",
    ),
  ...
  server.on("connection", (socket) =>
    socket.on("message", (raw) => {
      ...
      if (request.payload.threadId === "unsupported") {
        socket.send(
          JSON.stringify({ _tag: "Exit", requestId: request.id, exit: { _tag: "Failure" } }),
        );
        return;
      }
      socket.send(
        JSON.stringify({
          ...
          exit: {
            _tag: "Success",
            value: { thread: { id: "task", title: "Result" } },
          },
        }),
      );
    }),
